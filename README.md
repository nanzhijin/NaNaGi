...
⚠️ IP 地理定位: 本地开发 IP 127.0.0.1 无法定位 → 部署后真实外网 IP 即可工作
⚠️ ⚠️ Guest 用户细分: 当前所有 guest 共用 personId="guest" → 数据互相污染。P3 注册登录系统完成后每个 guest 拥有独立 personId 和 IWM Node。
...
| **P3** | 🔜 注册登录: personId分化 + IWM Cold Start + 前端表单 | 1 | 进行中 |
